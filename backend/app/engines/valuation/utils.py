...
def get_latest(df, key):
    ...
    try:
        if key in df.index:
            ...
            if pd.isna(val):
                # logger.debug(f"get_latest: {key} is NaN")
                return None
            return val
    except Exception:
        pass
...
def calculate_fcff(info: dict, income: pd.DataFrame, cashflow: pd.DataFrame, tax_rate: float) -> float:
    """
    ...
    try:
        ...
        logger.info(f"DEBUG: Interest Expense={interest_expense}, Tax Rate={tax_rate}")
        interest_addback = abs(interest_expense) * (1 - tax_rate)
        
        if ocf is not None:
            fcff = ocf + interest_addback + capex  # capex is negative
            logger.info(f"FCFF Calculation: OCF={ocf}, Interest Addback={interest_addback}, CapEx={capex}, FCFF={fcff}")
            return fcff
        
        # Method 2: Fallback to info (usually FCFE, not ideal)
        fcf_fallback = info.get('freeCashflow', 0)
        logger.warning(f"Using fallback FCF from info (may be FCFE): {fcf_fallback}")
        return fcf_fallback
...
def calculate_wacc(info, balance, income):
    """
    ...
    market_risk_premium = 0.055  # 5.5% (Updated from 5% for current market)
...
def get_forward_growth_rate(ticker: str, info: dict, sector: str) -> float:
    """
    ...
    try:
        ...
        # yfinance provides earnings/revenue growth in info
        ...
        if forward_earnings_growth and forward_earnings_growth > -0.5:  # Sanity check
            logger.info(f"{ticker}: Using Forward Earnings Growth = {forward_earnings_growth:.1%}")
            return forward_earnings_growth
            
    except Exception as e:
        logger.warning(f"Could not fetch analyst estimates for {ticker}: {e}")
    ...
    if forward_eps_growth and forward_eps_growth > -0.5:
        logger.info(f"{ticker}: Using Quarterly EPS Growth = {forward_eps_growth:.1%}")
        return forward_eps_growth
    
    # Method 3: Fallback to Historical Revenue Growth
    revenue_growth = info.get('revenueGrowth', 0.05)
    logger.info(f"{ticker}: Fallback to Historical Revenue Growth = {revenue_growth:.1%}")
